...
### Offline sales replay through `replay_sale`, never `log_sale`

Offline Phase 4 (2026-08-19), migration **0018**. `log_sale` is untouched and
must stay that way — it is the function every online sale depends on, and it is
wrong for replay in four measured ways: `sales` had no key to dedupe on, it
prices every line from `products.unit_price` (**today's** price), it RAISES
`Insufficient stock` and aborts, and it stamps `sold_by = auth.uid()` /
`created_at = now()`.

**Idempotency is the unique index on `(store_id, client_id)`, not a lookup** —
see D56. `lib/offline/sync.ts` never decides "I already sent that"; it sends and
believes `created` / `duplicate`. **A `duplicate` is a SUCCESS**, and is
reported to the cashier as "already recorded" rather than folded into the sent
count.

**An oversell lands, floors stock at 0, and writes a `stock_discrepancies`
row** carrying `units_sold`, `stock_available`, `shortfall` — returned by the
function so the UI raises it immediately. See D57: the software cannot prevent
an oversell that already happened on a shelf, only record it honestly.

**`replay_sale` trusts the client's `unit_price`** and this is deliberate: the
server cannot reconstruct what a customer was charged last week. It is rounded
...
describing an order of events that never happened); one failure never stops the
rest; and a queue entry is deleted only AFTER the server confirms — the opposite
order deletes a sale that never landed.

`stock_discrepancies` has **no INSERT policy at all**. Only the definer writes
