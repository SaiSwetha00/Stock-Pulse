...
import { StockProduct, PageView } from '../types';
import {
  Activity,
  AlertTriangle,
  ArrowRight,
...
export const DashboardPreview: React.FC<DashboardPreviewProps> = ({ onNavigate }) => {
  ...
  const handleSimulateScan = () => {
    ...
    setProducts((prev) =>
      prev.map((p) => (p.id === 'SKU-1029' ? { ...p, stock: p.stock + 5, status: 'warning' } : p))
    );
    setTimeout(() => setSimulatedScanMsg(null), 4000);
  };

  return (
    <section id="dashboard-preview" className="relative py-28 bg-[#0b0e15] overflow-hidden border-t border-[#edc155]/20 perspective-1500">
      {/* Background Flare */}
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-[#1d2027] border border-[#edc155]/40 text-[#edc155] hover:bg-[#edc155] hover:text-[#10131b] font-mono text-xs uppercase tracking-wider font-semibold transition-all flex items-center gap-2 cursor-pointer shadow-[0_0_15px_rgba(237,193,85,0.2)]"
            >
              <RefreshCw className="w-4 h-4 animate-spin-slow" /> TRIGGER AUTOMATED AI SCAN
            </button>
            <button
              onClick={() => onNavigate('dashboard')}
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#edc155] to-[#c9a037] text-[#10131b] font-mono text-xs uppercase tracking-wider font-semibold hover:shadow-[0_0_25px_rgba(237,193,85,0.5)] transition-all flex items-center gap-2 cursor-pointer"
            >
        ...
        {simulatedScanMsg && (
          <div className="mb-6 p-4 rounded-xl bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 font-mono text-xs flex items-center justify-between animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center gap-3">
              <CheckCircle2 className="w-5 h-5 text-emerald-400" />
              <span>{simulatedScanMsg}</span>
            </div>
            <span className="text-[10px] text-emerald-400/60 uppercase">JUST NOW</span>
          </div>
        )}
        ...
        <motion.div
          ...
          className="glass-panel rounded-2xl border border-[#edc155]/30 shadow-2xl overflow-hidden bg-[#10131b]/95 preserve-3d animate-3d-glow"
        >
          {/* Top Window Bar */}
          <div className="px-6 py-4 bg-[#181b23] border-b border-white/10 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="flex gap-1.5">
                <div className="w-3 h-3 rounded-full bg-[#93000a]" />
                <div className="w-3 h-3 rounded-full bg-[#edc155]" />
            ...
            <div className="flex items-center gap-2 font-mono text-xs">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-colors cursor-pointer ${
                  activeTab === 'all'
                    ? 'bg-[#edc155] text-[#10131b] font-bold'
                    : 'text-[#d1c5b0] hover:text-[#e0e2ed]'
                }`}
              >
          ...
          <div className="p-6 border-b border-white/5 bg-[#141720] flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-lg bg-[#10131b] border border-[#4e4636] text-xs text-[#e0e2ed] placeholder-[#d1c5b0]/40 outline-none focus:border-[#edc155]"
              />
            </div>
            ...
            <div className="flex items-center gap-6 font-mono text-xs text-[#d1c5b0]">
              <div>
                ...
                <span className="text-[#edc155] font-bold">$14,892.50</span>
              </div>
              <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#181b23]/80 font-mono text-[11px] uppercase tracking-wider text-[#d1c5b0]/60 border-b border-white/5">
              ...
              <tbody className="divide-y divide-white/5 font-sans text-xs">
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <tr
                      key={p.id}
                      className="hover:bg-[#1d2027]/80 transition-colors cursor-pointer"
                    >
                      <td className="py-4 px-6 font-mono font-semibold text-[#edc155]">
                        {p.id}
                      </td>
                      <td className="py-4 px-6 font-semibold text-[#e0e2ed]">
                        {p.name}
                      </td>
                      <td className="py-4 px-6 text-[#d1c5b0]/70">
                        {p.category}
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <div className="w-24 bg-[#10131b] h-2 rounded-full overflow-hidden border border-white/10">
                            <div
                              className={`h-full ${
                                stockPct < 25
                                  ? 'bg-[#93000a]'
          ...
          {/* Footer Bar */}
          <div className="p-4 bg-[#141720] border-t border-white/5 flex items-center justify-between font-mono text-[11px] text-[#d1c5b0]/60">
            <div>Click any product row to view deep telemetry inspection logs</div>
            <div className="text-[#edc155]">Synced with Cold-Chain Probe #7</div>
          </div>
        </motion.div>
      </div>
    </section>
