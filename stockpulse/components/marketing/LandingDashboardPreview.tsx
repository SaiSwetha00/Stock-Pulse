...
export default function LandingDashboardPreview() {
  ...
  return (
    <section id="dashboard-preview" className="relative py-32 sp-band-gradient-a overflow-hidden border-t border-white/8 perspective-1500">
      {/* Background Flare */}
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="px-6 py-3 rounded-xl bg-[var(--sp-surface-card)] border border-[var(--sp-gold)]/40 text-[var(--sp-gold)] hover:bg-[var(--sp-gold)] hover:text-black font-mono text-xs uppercase tracking-wider font-semibold transition-all flex items-center gap-2 cursor-pointer shadow-[0_0_15px_rgba(201,162,39,0.2)]"
            >
              <RefreshCw className="w-4 h-4" /> TRIGGER AUTOMATED AI SCAN
            </button>
            <Link
              href="/dashboard"
              className="px-7 py-3 rounded-xl bg-gradient-to-r from-[var(--sp-gold)] to-[var(--sp-gold-deep)] text-black font-mono text-xs uppercase tracking-wider font-semibold hover:shadow-[0_0_25px_rgba(201,162,39,0.5)] transition-all flex items-center gap-2 cursor-pointer"
            >
        ...
        {simulatedScanMsg && (
          <div className="mb-6 p-4 rounded-xl bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 font-mono text-xs flex items-center justify-between">
        ...
        <motion.div
          ...
          className="glass-panel rounded-2xl border border-[var(--sp-gold)]/30 shadow-2xl overflow-hidden bg-black/95 preserve-3d animate-3d-glow"
        >
          {/* Top Window Bar */}
          <div className="px-7 py-5 bg-[var(--sp-surface-alt)] border-b border-white/10 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 font-mono text-xs">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-colors cursor-pointer ${
                  activeTab === 'all' ? 'bg-[var(--sp-gold)] text-black font-bold' : 'text-[#d1c5b0] hover:text-[#e0e2ed]'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-colors cursor-pointer flex items-center gap-1.5 ${
                  activeTab === 'warning' ? 'bg-[#93000a] text-white font-bold' : 'text-[#ffb4ab] hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition-colors cursor-pointer ${
                  activeTab === 'optimal' ? 'bg-emerald-600 text-white font-bold' : 'text-emerald-400 hover:text-white'
                }`}
              >
                OPTIMAL
              </button>
          ...
          <div className="p-7 border-b border-white/5 bg-[var(--sp-surface-elevated)] flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 rounded-lg bg-black border border-[#4e4636] text-xs text-[#e0e2ed] placeholder-[#d1c5b0]/40 outline-none focus:border-[var(--sp-gold)]"
              />
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[var(--sp-surface-alt)]/80 font-mono text-[11px] uppercase tracking-wider text-[#d1c5b0]/60 border-b border-white/5">
                ...
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-[var(--sp-surface-card)]/80 transition-colors cursor-pointer">
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <div className="w-24 bg-black h-2 rounded-full overflow-hidden border border-white/10">
                            <div
                              className={`h-full ${
                                stockPct < 25 ? 'bg-[#93000a]' : stockPct < 50 ? 'bg-[var(--sp-gold)]' : 'bg-emerald-500'
                              }`}
                      ...
                      <td className="py-4 px-6 font-mono text-[11px] text-emerald-400">{p.temperature}</td>
                      <td className="py-4 px-6">
                        <span
          ...
          {/* Footer Bar */}
          <div className="p-5 bg-[var(--sp-surface-elevated)] border-t border-white/5 flex items-center justify-between font-mono text-[11px] text-[#d1c5b0]/60">
